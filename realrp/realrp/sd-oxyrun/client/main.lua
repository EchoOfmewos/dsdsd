...
function hasPackageChanged()
    local player = PlayerPedId()
        hasPackage = SD.HasItem(Config.Item, 1)
        if hasPackage then
...
RegisterNetEvent('sd-oxyrun:client:start', function()
    if Cooldown then
        SD.SD.ShowNotification(Lang:t("error.done_recently"), 'error')
    else
...
local function runStart()
    Wait(2000)
    sendEmail(Lang:t("mailstart.message"))
end

local function runEnd()
    Wait(2000)
    sendEmail(Lang:t("mailfinish.message"))
end
