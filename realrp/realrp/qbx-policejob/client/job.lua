...
PDStations = {
    vector3(481.10916137695, -1010.7631835938, 26.273202896118), -- MRPD
    vector3(380.79, -1614.57, 29.29), -- DAVIS
    vector3(840.55, -1290.2, 28.65), -- TROOPER STATION
    vector3(1810.0775146484, 3678.916015625, 34.18928527832), -- SANDY
    ...
    if PlayerJob.name == "police" then
        if canOpen then
            ...
            if args[1] ~= nil then
                ...
                TriggerServerEvent('qbx-policejob:server:CreateEvidence', " 1 | Drawer "..slot, slot)
            else
...
RegisterNetEvent('qbx-policejob:client:CreateEvidence', function(stashID)
    exports.ox_inventory:openInventory('stash', {id=stashID})
    print('evidence opened: '..tostring(stashID))
end)
...
local function SetWeaponSeries()
    for k, _ in pairs(Config.Items.items) do
        if k < 6 then
            Config.Items.items[k].metadata.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
        end
    end
end
...
RegisterNetEvent('police:client:takeoffmask', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('police:client:takeoffmaskc', function()
...
RegisterNetEvent('qb-police:client:openArmoury', function()
   ...
   exports.ox_inventory:openInventory('shop', {type = 'policearmory', id = 1})
end)
...
RegisterNetEvent('qb-police:client:openTrash_', function()
    exports.ox_inventory:openInventory('stash', { id = 'policetrash_'})
end)
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['qb-target']:AddBoxZone("PoliceTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-police:client:openTrash_",
                        --event = 'qb-police:client:openStash',
                        icon = "fas fa-trash",
                        label = "Open Trash",
                        job = "police",
