...
Config.Framework = 'QBCORE' -- ESX | QBCORE | OX
Config.inventory = 'OX' -- ESX | QBCORE | OX
Config.debug = false -- enables some debug prints
Config.target = true -- enable if using target/third-eye system
Config.targetSystem = 'ox_target' -- ox_target | qb-target | qtarget
Config.fareMin = 14 -- min possible number divided by ten to then be multiplied by distance of route then divided by ten again to work each fare payout - whole numbers only, no decimal points.
Config.fareMax = 20 -- max possible number divided by ten to then be multiplied by distance of route then divided by ten again to work each fare payout - whole numbers only, no decimal points.
Config.fuelPay = 1.0 -- Fuel compensation
Config.maxFares = 20 -- max fares that can be queued up
Config.NewFareTimer = 1 -- time in minutes that new fares are generated
Config.useOTSkills = true -- set to true if using OT_skills
...
Config.chargeForTaxi = false -- enable / disable charging to rent taxi.
Config.givekey = true
Config.jobcheck = false
Config.job = 'taxi' -- job that can use taxi
Config.grade = 1 -- min grade for job
Config.CleanupOnDisconnect = false -- enable / disable vehicle deletion on player crash/disconnect
