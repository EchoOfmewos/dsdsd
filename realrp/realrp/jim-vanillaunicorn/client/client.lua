...
local Ped, Props, Targets, Blips, jacuzzilist, tseat, alcoholCount, inJacuzzi = nil, {}, {}, {}, {}, {}, 0, 0, false
...
RegisterNetEvent('jim-vanillaunicorn:washHands', function(data)
	lookEnt(data.coords)
	if progressBar({ label = Loc[Config.Lan].progressbar["washing_hands"], time = 5000, cancel = true, dict = "mp_arresting", anim = "a_uncuff", flag = 16, icon = data.item }) then
...
RegisterNetEvent('jim-vanillaunicorn:Crafting:MakeItem', function(data)
	...
	elseif data.header == Loc[Config.Lan].menu["oven"] then
		bartext = Loc[Config.Lan].progress["cooking"] bartime = 2000 animDictNow = "amb@prop_human_bbq@male@base" animNow = "base"
	elseif data.header == Loc[Config.Lan].menu["cocktail"] then
		bartext = Loc[Config.Lan].progress["mixing"] bartime = 2000 animDictNow = "anim@heists@prison_heiststation@cop_reactions" animNow = "cop_b_idle"
	end
	if progressBar({ label = bartext..QBCore.Shared.Items[data.item].label, time = bartime, cancel = true, dict = "mp_ped_interaction", anim = "handshake_guy_a", flag = 8, icon = data.item }) then
		CraftLock = false
...
RegisterNetEvent('jim-vanillaunicorn:Crafting:MultiCraft', function(data)
	...
	for k in pairsByKeys(success) do
		Menu[#Menu + 1] = {
			...
			event = "jim-vanillaunicorn:Crafting:MakeItem", args = { item = data.item, craft = data.craft, craftable = data.craftable, header = data.header, anim = data.anim, amount = k },
			params = { event = "jim-vanillaunicorn:Crafting:MakeItem", args = { item = data.item, craft = data.craft, craftable = data.craftable, header = data.header, anim = data.anim, amount = k } }
		}
	end
	if Config.Menu == "ox" then	exports.ox_lib:registerContext({id = 'Crafting', title = data.header, position = 'top-right', options = Menu })	exports.ox_lib:showContext("Crafting")
	elseif Config.Menu == "qb" then	exports['qb-menu']:openMenu(Menu) end
