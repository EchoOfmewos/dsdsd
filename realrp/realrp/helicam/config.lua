...
Config.InstantCameraLock = true -- If true, the camera will immediately lock on to the target instead of requiering a grace period
...
Config.WhitelistedJobs = { ['police'] = 0, ['ambulance'] = 0 } -- If false, the script doesn't check for jobs. If you want to check for jobs add a table where the job is the key, and the value is the job level like so: Config.WhitelistedJobs = { ['police'] = 2, ['ambulance'] = 5 }
...
Config.ShowPostalCodes = true -- Shows postals while in the camera (This is a little resource heavy)
...
Config.PostalFile = "ocrp-postals.json" -- MUST be a JSON file! (Note: The file must be loaded inside the fxmanifest of Config.PostalResource for the helicam script to be able to read it)
...
-- MI = Miles
Config.Units = {
    Speed = "MPH", -- The speed of the helicopter                               (unit type: speed)
