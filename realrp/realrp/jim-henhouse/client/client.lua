	...
	if Config.MLO == "SM" then
		...
		Targets["HenFridge2"] =
			exports['qb-target']:AddBoxZone("HenFridge2", vec3(-301.63, 6272.5, 31.49-1), 1.55, 0.4, { name="HenFridge2", heading = 313.0, debugPoly=Config.Debug, minZ=30.49, maxZ=32.69, },
				{ options = { {  event = "jim-henhouse:Shop", icon = "fas fa-archive", label = Loc[Config.Lan].target["drink_fridge"], shop = Ingredients.DrinkItems, job = loc.job, shopname = "smdrinkfridge", coords = vec3(-301.63, 6272.5, 31.49), }, }, distance = 1.0 })
		Targets["HenFridge3"] =
			exports['qb-target']:AddBoxZone("HenFridge3", vec3(-300.04, 6273.75, 31.38-1), 0.8, 1.0, { name="HenFridge3", heading = 223.06, debugPoly=Config.Debug, minZ = 30.5, maxZ = 32.80, },
				{ options = { {  event = "jim-henhouse:Shop", icon = "fas fa-archive", label = Loc[Config.Lan].target["food_fridge"], shop = Ingredients.FoodItems, job = loc.job, shopname = "smfoodfridge", coords = vec3(-300.04, 6273.75, 31.38), }, }, distance = 2.0 })
...
RegisterNetEvent('jim-henhouse:Crafting:MakeItem', function(data)
	...
	for i = 1, #Crafting.Cocktails do
		for k in pairs(Crafting.Cocktails[i]) do
			if data.item == k then
				bartext = Loc[Config.Lan].progressbar["progress_mix"] bartime = 2000
	...
	for i = 1, #Crafting.Food do
		for k in pairs(Crafting.Food[i]) do
			if data.item == k then
				bartext = Loc[Config.Lan].progressbar["progress_cook"] bartime = 2000
	...
	for i = 1, #Crafting.Beer do
		for k in pairs(Crafting.Beer[i]) do
			if data.item == k then
				bartext = Loc[Config.Lan].progressbar["progress_pour"] bartime = 2000
