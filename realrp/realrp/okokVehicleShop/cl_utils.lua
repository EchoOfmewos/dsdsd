...
function TextUI(text, color, pos)
	if text ~= nil then
		exports['okokTextUI']:Open(text, color, pos)
	else
...
function startCamAnimation()
	if selectedVehicle ~= nil and vehicleShopIsOpen then
		createCameras()
		if #totalCams > 1 then
			local i = 2
			local startCam = totalCams[1]
			local nextCam = totalCams[i]
			local stopLoop = false

			while vehicleShopIsOpen do
				local timePassed = 0
				while timePassed < Config.TimeBetweenTransition do
					Citizen.Wait(100)
					timePassed = timePassed + 100
					if changeVehicle or not vehicleShopIsOpen then
						stopLoop = true
						break
					end
				end
				...
				SetCamActiveWithInterp(nextCam, startCam, Config.TransitionTime, 3, 1)
				startCam = nextCam
				nextCam = totalCams[i]
			end
			changeVehicle = false
	...
	while doingMission do
			...
			if hasTrailer then
				...
				if not isTowed then
					...
					if distance < 100 then
						waitMore = false
						DrawMarker(Config.TowMarker.id, spawnTruckMission.x, spawnTruckMission.y, spawnTruckMission.z, 0, 0, 0, 0, 0, 0, Config.TowMarker.size.x, Config.TowMarker.size.y, Config.TowMarker.size.z, 31, Config.TowMarker.color.r, Config.TowMarker.color.g, Config.TowMarker.color.b, Config.TowMarker.bobUpAndDown, Config.TowMarker.faceCamera, 2, Config.TowMarker.rotate, 0, 0, Config.TowMarker.drawOnEnts)
						if distance < 5 then
							break
						end
					end
						...
						if distanceTruck < 1 then
							...
							if IsControlJustReleased(0, Config.Key) then
								...
								if distanceOrder < maxDistance then
									isTowed = true
									if not shopHasBigVehicles then
										AttachEntityToEntity(missionVehicle, missionTruck, GetEntityBoneIndexByName(missionTruck, 'bodyshell'), towCoords.xPos, towCoords.yPos, towCoords.zPos, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
									else
										AttachEntityToEntity(missionVehicle, missionTrailer, GetEntityBoneIndexByName(missionTruck, 'bodyshell'), towCoords.xPos, towCoords.yPos, towCoords.zPos, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
									end
		...
		if not shown and inZone then
			...
			if Config.UseOkokTextUI then
				TextUI(Config.TextUI['tow'].text, Config.TextUI['tow'].color, Config.TextUI['tow'].side)
			else
				QBCore.Functions.Notify(Config.HelpNotification['tow'].text, Config.HelpNotification['tow'].type, Config.HelpNotification['tow'].time)
			end
