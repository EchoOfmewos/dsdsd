...
RegisterNetEvent('jim-pizzathis:Crafting:MakeItem', function(data)
	...
	for i = 1, #Crafting.ChoppingBoard do
		for k, v in pairs(Crafting.ChoppingBoard[i]) do
			if data.item == k then
				bartext = Loc[Config.Lan].progress["mixing"] bartime = 2000
	...
	for i = 1, #Crafting.Oven do
		for k, v in pairs(Crafting.Oven[i]) do
			if data.item == k then
				bartext = Loc[Config.Lan].progress["cooking"] bartime = 2000
	...
	for i = 1, #Crafting.Beer do
		for k, v in pairs(Crafting.Beer[i]) do
			if data.item == k then
				bartext = Loc[Config.Lan].progress["pouring"] bartime = 2000
				animDictNow = "mp_ped_interaction" animNow = "handshake_guy_a"
			end
		end
	end
	if ItemMake == "pizzadough" then
		bartext = Loc[Config.Lan].progress["grabbing"]..QBCore.Shared.Items[ItemMake].label	bartime = 2000
		animDictNow = "anim@heists@prison_heiststation@cop_reactions" animNow = "cop_b_idle"
	elseif ItemMake == "coffee" then
		bartext = Loc[Config.Lan].progress["pouring"]..QBCore.Shared.Items[ItemMake].label bartime = 2000
		animDictNow = "mp_ped_interaction" animNow = "handshake_guy_a"
	end
	...
	if progressBar({ label = bartext..QBCore.Shared.Items[data.item].label, time = bartime, cancel = true, dict = animDictNow, anim = animNow, flag = 1, icon = data.item }) then
		CraftLock = false
...
RegisterNetEvent('jim-pizzathis:Crafting:MultiCraft', function(data)
	...
	for k in pairsByKeys(success) do
		Menu[#Menu + 1] = {
			...
			event = "jim-pizzathis:Crafting:MakeItem", args = { item = data.item, craft = data.craft, craftable = data.craftable, header = data.header, anim = data.anim, amount = k },
			params = { event = "jim-pizzathis:Crafting:MakeItem", args = { item = data.item, craft = data.craft, craftable = data.craftable, header = data.header, anim = data.anim, amount = k } }
		}
	end
	if Config.Menu == "ox" then	exports.ox_lib:registerContext({id = 'Crafting', title = data.header, position = 'top-right', options = Menu })	exports.ox_lib:showContext("Crafting")
	elseif Config.Menu == "qb" then	exports['qb-menu']:openMenu(Menu) end
