...
RegisterNetEvent('jixel-pearls:Crafting:MakeItem', function(data)
	if data.header == Loc[Config.Lan].menu["header_cocktail"] then -- Cocktail
		bartext = Loc[Config.Lan].progressbar["progress_mix"]..QBCore.Shared.Items[data.item].label	bartime = 1000
			animDictNow = "mp_ped_interaction" animNow = "handshake_guy_a"
	elseif data.header == Loc[Config.Lan].menu["header_beer"] then -- Beer
		bartext = Loc[Config.Lan].progressbar["progress_pour"]..QBCore.Shared.Items[data.item].label bartime = 1000
		animDictNow = "mp_ped_interaction" animNow = "handshake_guy_a"
	...
	elseif data.header == Loc[Config.Lan].menu["header_oven"] then -- Oven
		bartext = Loc[Config.Lan].progressbar["progress_cook"]..QBCore.Shared.Items[data.item].label bartime = 1000
		animDictNow = "amb@prop_human_bbq@male@base" animNow = "base"
	elseif data.header == Loc[Config.Lan].menu["header_fryer"] then -- Frying
		bartext = Loc[Config.Lan].progressbar["progress_fry"]..QBCore.Shared.Items[data.item].label bartime = 1000
		animDictNow = "amb@prop_human_bbq@male@base" animNow = "base"
	elseif data.header == Loc[Config.Lan].menu["header_sushi"] then -- Sushi
		bartext = Loc[Config.Lan].progressbar["progress_rolling"]..QBCore.Shared.Items[data.item].label bartime = 1000
	...
	elseif data.header == Loc[Config.Lan].menu["header_chopboard"] then -- ChopBoard
		bartext = Loc[Config.Lan].progressbar["progress_chop"]..QBCore.Shared.Items[data.item].label bartime = 1000
		animDictNow = "anim@heists@prison_heiststation@cop_reactions" animNow = "cop_b_idle"
	else
		bartext = Loc[Config.Lan].progressbar["progress_cook"] bartime = 1000
		animDictNow = "amb@prop_human_bbq@male@base" animNow = "base"
	end
