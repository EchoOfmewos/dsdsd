...
function GetKeyLabel(commandHash)
    ...
    local keyLabel = TranslateKey(key)
    if not keyLabel then
        error("Attempted to get key label, but the label was not found, key code was: "..tostring(key))
        keyLabel = "unk"
    end
...
local function CanPlayerSitInSeat(coords, heading, entity)
	...
	if hit == 0 or entityHit == entity then
		return true
	else
		return false
	end
