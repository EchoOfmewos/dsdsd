...
RegisterNetEvent('qb-radialmenu:client:spawnboat', function()
    local PlayerData = QBCore.Functions.GetPlayerData()
    if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" then
        local ped = PlayerPedId()
        if IsPedSwimmingUnderWater(ped) or IsPedSwimming(ped) or IsEntityInWater(ped) then
            QBCore.Functions.Progressbar("take_out_boat",'Pulling out boat...', math.random(10000, 20000), false, true, {
            ...
            QBCore.Functions.Notify('You are not in water', 'error')
        end
    end
end)
