...
Config.Inventory = {
    ...
    QS = false, -- YOU MUST USE CONFIG.CALIBERS FOR QS-INVENTORY
...
Config.DB = { -- this config sets what table you are using to store vehicle evidence
    ...
    Identifiers = 'players', -- if you use ESX or a standalone framework and need to generate citizenids, fingerprints, and bloodtypes, set this to your custom table
}
...
Config.GSR = {
    MinShotsStatus = 2, -- the amount of times someone can fire nearby a player before they get a gunpowder status effect
...
Config.EvidenceCleanup = { 
    RemoveFingerprints = { -- these items will remove fingerprints and can be combined with the remove blood items   *********IF YOU ARE USING OX_INVENTORY YOU MUST ADD A SERVER EXPORT FOR IT TO BE USEABLE************ 
        ...
        ['rag'] = {useable = true}, -- this item will allow you to wipe down the weapon if you use the /cleanweapon command  *********IF YOU ARE USING OX_INVENTORY YOU MUST ADD A SERVER EXPORT FOR IT TO BE USEABLE************ 
    },
...
Config.Breathalyzer = {
    ...
    EventTriggers = {
        [1] = {event = 'jim-consumables:server:toggleItem', type = 'server', alocholArgPos = 2, alocholArgSubField = 'item', alcoholArgValue = 'margarita', cidArgPos = nil},
    },
}
...
Config.DrugTesting = {
    Enabled = true,
    UsingQBSR = false, -- set to true if you are using basic qb-smallresources, if not you will need to specify an event trigger or edit your consumables resource
...
of the access tool by non-police players. These events are current set up to trigger the native qbx-policejobjob
...
manually define which components DO NOT have gloves. If you have not modifies the arm components for mp_freemode
...
Config.CarWash = {
    Settings = {
        usePoly = false, -- will use polyzone to create car washes, false it will distance check
...
This table allows us to define the custom functions which enable framework compatability and allows you to
...
Config.Functions = {
    ...
    GetCaliber = function(weapon)
        if Config.UseConfigCalibers or Config.Inventory.QS then return Config.Calibers[weapon] end
