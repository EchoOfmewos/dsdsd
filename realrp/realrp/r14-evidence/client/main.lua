        ...
        if v.type == 'client' and v.event ~= 'evidence:client:SetStatus' then
            RegisterNetEvent(v.event, function(arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg9, arg10)
                ...
                if Config.Debug.PrintEventTriggerArgs.enabled then
                    ...
                    print(('Event Name: ^1%s^0'):format(v.event))
                    print(('Current Argument (alcoholArgPos): ^1%s^0'):format(v.alcoholArgPos))
                    print(('Value Found At Argument %s: ^1%s^0'):format(v.alcoholArgPos, type(v.alcoholArgValue) == 'table' and json.encode(args[v.alcoholArgPos], {indent = true}) or args[v.alcoholArgPos]))
                    print(('Value Being Checked Against: ^1%s^0'):format(type(v.alcoholArgValue) == 'table' and json.encode(v.alcoholArgValue, {indent = true}) or v.alcoholArgValue))
                    print(('Args Being Received By Event: ^1{^0'):format(json.encode(debugArgs, {indent = true})))
                    for k, v in pairs(debugArgs) do print(('^1    Argument %s:  %s^0'):format(v.key, type(args[v.value]) == 'table' and json.encode(v.value, {indent = true}) or v.value)) end
                    print('^1}^0')
                end
        ...
        if v.type == 'client' then
            RegisterNetEvent(v.event, function(arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg9, arg10)
                ...
                if Config.Debug.PrintEventTriggerArgs.enabled then
                    ...
                    print(('Event Name: ^1%s^0'):format(v.event))
                    print(('Current Argument (drugArgPos): ^1%s^0'):format(v.drugArgPos))
                    print(('Value Found At Argument %s: ^1%s^0'):format(v.drugArgPos, type(v.drugArgValue) == 'table' and json.encode(args[v.drugArgPos], {indent = true}) or args[v.drugArgPos]))
                    print(('Value Being Checked Against: ^1%s^0'):format(type(v.drugArgValue) == 'table' and json.encode(v.drugArgValue, {indent = true}) or v.drugArgValue))
                    print(('Args Being Received By Event: ^1{^0'):format(json.encode(debugArgs, {indent = true})))
                    for k, v in pairs(debugArgs) do print(('^1    Argument %s:  %s^0'):format(v.key, type(args[v.value]) == 'table' and json.encode(v.value, {indent = true}) or v.value)) end
                    print('^1}^0')
                end
    ...
    if noteinput ~= nil then
        if not data.note then 
            data.item[inventoryMetadata].notes[#data.item[inventoryMetadata].notes+1] = {
                ...
                created = {name = ('%s %s'):format(PlayerData.firstname, PlayerData.lastname), timestamp = collection},
            }
        else
            data.item[inventoryMetadata].notes[data.note].text = noteinput.text
            data.item[inventoryMetadata].notes[data.note].edited = {name = ('%s %s'):format(PlayerData.firstname, PlayerData.lastname), timestamp = collection}
        end
